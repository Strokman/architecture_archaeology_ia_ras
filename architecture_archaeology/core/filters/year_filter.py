class RangeDatesFilterBase:
    ...
    def filter_dating(self, queryset, name, value):
        ...
        return queryset.filter(year_min__lt=filter_period['end'], year_max__gt=filter_period['start']) | queryset.filter(year_min=filter_period['end'], year_max=filter_period['start'])